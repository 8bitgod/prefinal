...
console = cp5.addTextarea("txt").setPosition(5,20).setSize(150, 255).setFont(createFont("proggy",13)).setColor(color(255,128,0)).setColorBackground(color(0)); 
...
sensor1 = cp5.addTextarea(" txt").setPosition(190,125).setSize(110,34).setFont(createFont("proggy",9)).setColor(color(255,128,0)).setColorBackground(color(0));
...
sliderSpeed = cp5.addSlider("speed").setPosition(190, 30).setRange(0,250).setNumberOfTickMarks(251).snapToTickMarks(true).showTickMarks(false);  
...
void keyPressed() 
{
  ...
  {      
    ...
    else if(checkKey(KeyEvent.VK_I)){INC();}
    
    else if(checkKey(KeyEvent.VK_J)){DEC();}

    else if(checkKey(KeyEvent.VK_ENTER)){Send();}
...

{
  ...
  if(!speedLock){speed =int(sliderSpeed.getValue());}
...
void INC()
{
  x=int(sliderSpeed.getValue());
  x++;
  cp5.getController("speed").setValue(x);
}
